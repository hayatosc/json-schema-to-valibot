...
async function createValidator(valibotCode: string): Promise<(data: any) => boolean> {
  const tempFile = path.join(
    process.cwd(),
    `temp-validator-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.mjs`,
  )

  try {
    const cleanCode = valibotCode
      .replace(/import \* as v from ['"]valibot['"];?\s*/, '')
      ...
      try {
        ...
        for (const testCase of schemaTest.tests) {
          this.results.push({
            ...
            actual: false,
            status: 'ERROR',
            error: `Schema conversion failed: ${conversionError instanceof Error ? conversionError.message : String(conversionError)}`,
          })
        }
      }
    }
  ...
  private printReport(): void {
    ...
    if (errorResults.length > 0) {
      ...
      errorResults.forEach((result) => {
        const truncatedSchema =
          result.schema.length > 38 ? result.schema.substring(0, 35) + '...' : result.schema
        const truncatedTest =
          result.test.length > 38 ? result.test.substring(0, 35) + '...' : result.test
    ...
    if (failedResults.length > 0) {
      ...
      failedResults.slice(0, 50).forEach((result) => {
        ...
        const truncatedSchema =
          result.schema.length > 28 ? result.schema.substring(0, 25) + '...' : result.schema
        const truncatedTest =
          result.test.length > 33 ? result.test.substring(0, 30) + '...' : result.test
        const truncatedInput =
          result.input.length > 18 ? result.input.substring(0, 15) + '...' : result.input
